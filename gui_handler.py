...
class GUIHandler:
    def __init__(self, video_handler, motion_detector, roi_manager, image_saver, config):
        ...
        self.root.geometry("1280x720")  # Задайте желаемый начальный размер
    ...
    def on_closing(self):
        ...
        self.video_handler.release() # Освобождаем ресурсы ffmpeg
        self.root.destroy()
    ...
    def draw_zones(self, frame):
        ...
        cv2.rectangle(frame, (x1, y1), (x1 + w1, y1 + h1), (0, 0, 255), 2) # Красная зона 1
        cv2.rectangle(frame, (x2, y2), (x2 + w2, y2 + h2), (0, 0, 255), 2) # Красная зона 2
        cv2.rectangle(frame, (rx, ry), (rx + rw, ry + rh), (0, 255, 0), 2) # Зеленая ROI
    ...
    def draw_resize_handles(self, frame, rect):
        ...
        for hx, hy in handles:
            cv2.rectangle(frame, (hx, hy), (hx + self.resize_handle_size, hy + self.resize_handle_size), (255, 255, 0), -1)

    def update_frame(self):
        frame_rgb = self.video_handler.get_frame_rgb()
        print(f"Получен кадр RGB: {frame_rgb is not None}")
        if frame_rgb is not None:
            frame_with_zones = self.draw_zones(frame_rgb.copy())
            frame_pil = Image.fromarray(frame_with_zones)
        ...
        self.root.after(30, self.update_frame) # Обновление примерно каждые 30 мс
